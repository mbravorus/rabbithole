...
export function normalizedImageDimensions(width, height, maxLongEdge = CLIPBOARD_IMAGE_MAX_LONG_EDGE) {
  ...
  return { width: Math.max(1, Math.round(width * scale)), height: Math.max(1, Math.round(height * scale)), downscaled: scale < 1 };
}

/** @param {unknown} sourceType @param {number} [encodedBytes] */
export function clipboardImageOutput(sourceType, encodedBytes = 0) {
  const jpeg = String(sourceType || "").toLowerCase() === "image/jpeg" ||
...
export async function normalizeClipboardImage(source) {
  if (!(source instanceof Blob) || !String(source.type || "").toLowerCase().startsWith("image/")) {
  ...
  try {
    ...
    try {
      ...
      if (sizeError) {
        const error = new Error(sizeError === "asset_too_large" ? "Pasted image exceeds 20 MB." : "Pasted image is too large.");
        error.code = sizeError;
        throw error;
      }
...
async function decodeImage(source) {
  ...
  try {
    image.src = url;
    await image.decode();
    return { image, width: image.naturalWidth, height: image.naturalHeight, release: function(){ URL.revokeObjectURL(url); } };
  } catch (error) {
